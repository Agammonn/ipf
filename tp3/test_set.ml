...
effectue un test, le v̩rifie et affiche le r̩sultat
     *)
    ...
    (** affichage pour les r̩sultats bool̩en *)
...
(**
    let fprintf_set fmt s =
      Format.fprintf fmt "[%a]"
        ...
        s
        *)
    (** affichage pour les r̩sultats de type int_set *)

    let fprintf_exn fmt e = Format.fprintf fmt "%s" (Printexc.to_string e)
    (** affichage pour les r̩sultats de type exn (exception) *)
  ...
  [ "empty_is_empty", (fun () -> (is_empty empty)), true, fprintf_bool;
    ...
    "union_equal_1", (fun () -> equal (union (add 1 (add 3 empty)) (add 2 (add 5 empty))) (add 1 (add 2 (add 3 (add 5 empty ))))), true, fprintf_bool;
  ]

    let tests_int = [
     "fold", (fun () ->fold (fun a b -> a + b) (add 1 (add 3 empty)) 2), 6, fprintf_int;
    "get_min_1", (fun () -> get_min (add 1 (add 3 empty))), 1, fprintf_int;
    "get_min_2", (fun () -> get_min (add 4 (add 3 empty))), 3, fprintf_int;
      ]
...
let main () =
  let module TList = Test(MakeListe(Entier)) in
  let module TAbr = Test(MakeAbr(Entier)) in
  let module TInt = Test(MakeAbr(struct type t = string let compare = Pervasive.string end )) in
